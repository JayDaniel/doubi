...
Download_ocserv(){
	...
	if [[ -e ${file} ]]; then
		mkdir "${conf_file}"
		wget --no-check-certificate -N -P "${conf_file}" "https://raw.githubusercontent.com/ihomelp07/doubi/master/other/ocserv.conf"
...
Service_ocserv(){
	if ! wget --no-check-certificate https://raw.githubusercontent.com/ihomelp07/doubi/master/other/ocserv_debian -O /etc/init.d/ocserv; then
...
Installation_dependency(){
	...
	if [[ ${release} = "centos" ]]; then
		...
		if [[ $? = 0 ]]; then
			...
			wget --no-check-certificate -O "/etc/apt/sources.list" "https://raw.githubusercontent.com/ihomelp07/doubi/master/sources/us.sources.list"
			apt-get update
			apt-get install vim net-tools pkg-config build-essential libgnutls28-dev libwrap0-dev liblz4-dev libseccomp-dev libreadline-dev libnl-nf-3-dev libev-dev gnutls-bin -y
...
Update_Shell(){
	...
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/ihomelp07/doubi/master/ocserv.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	if [[ $sh_new_type == "softs" ]]; then
		...
		wget -N --no-check-certificate "https://raw.githubusercontent.com/ihomelp07/doubi/master/ocserv.sh" && chmod +x ocserv.sh
	fi
