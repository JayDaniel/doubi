...
Service_ps(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/ihomelp07/doubi/master/other/pserver_centos" -O /etc/init.d/pserver; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/ihomelp07/doubi/master/other/pserver_debian" -O /etc/init.d/pserver; then
			echo -e "${Error} Peerflix Server服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/pserver
...
Update_Shell(){
	...
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/ihomelp07/doubi/master/pserver.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	if [[ ${sh_new_type} == "softs" ]]; then
		...
		wget -N --no-check-certificate "https://raw.githubusercontent.com/ihomelp07/doubi/master/pserver.sh" && chmod +x pserver.sh
	fi
