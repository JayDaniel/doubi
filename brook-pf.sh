...
Service_brook(){
	if [[ "${Download_type}" == "1" ]]; then
		...
		if [[ ${release} = "centos" ]]; then
			if ! wget --no-check-certificate https://raw.githubusercontent.com/ihomelp07/doubi/master/other/brook-pf_centos -O /etc/init.d/brook-pf; then
				echo -e "${Error} Brook服务 管理脚本下载失败 !" && exit 1
			fi
			chmod +x /etc/init.d/brook-pf
			chkconfig --add brook-pf
			...
			if ! wget --no-check-certificate https://raw.githubusercontent.com/ihomelp07/doubi/master/other/brook-pf_debian -O /etc/init.d/brook-pf; then
				echo -e "${Error} Brook服务 管理脚本下载失败 !" && exit 1
			fi
			chmod +x /etc/init.d/brook-pf
			update-rc.d -f brook-pf defaults
...
Update_Shell(){
	...
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/ihomelp07/doubi/master/brook-pf.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	if [[ ${sh_new_type} == "softs" ]]; then
		...
		wget -N --no-check-certificate "https://raw.githubusercontent.com/ihomelp07/doubi/master/brook-pf.sh" && chmod +x brook.sh
	fi
