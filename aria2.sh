...
Download_aria2_conf(){
	mkdir "${file}" && cd "${file}"
	wget --no-check-certificate -N "https://raw.githubusercontent.com/JayDaniel/doubi/master/other/Aria2/aria2.conf"
	[[ ! -s "aria2.conf" ]] && echo -e "${Error} Aria2 配置文件下载失败 !" && rm -rf "${file}" && exit 1
	wget --no-check-certificate -N "https://raw.githubusercontent.com/JayDaniel/doubi/master/other/Aria2/dht.dat"
...
Service_aria2(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/JayDaniel/doubi/master/other/aria2_centos -O /etc/init.d/aria2; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/JayDaniel/doubi/master/other/aria2_debian -O /etc/init.d/aria2; then
			echo -e "${Error} Aria2服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/aria2
...
Update_Shell(){
	...
	if [[ -z ${softs_domain} ]]; then
		softs_domain="https://raw.githubusercontent.com/JayDaniel/doubi/master/"
	else
	...
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/JayDaniel/doubi/master/aria2.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	if [[ ${sh_new_type} == "softs" ]]; then
		...
		wget -N --no-check-certificate "https://raw.githubusercontent.com/JayDaniel/doubi/master/aria2.sh" && chmod +x aria2.sh
	fi
