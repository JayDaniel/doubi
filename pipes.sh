...
Service_pipes(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/ihomelp07/doubi/master/other/pipes_centos -O /etc/init.d/pipes; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/ihomelp07/doubi/master/other/pipes_debian -O /etc/init.d/pipes; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/pipes
