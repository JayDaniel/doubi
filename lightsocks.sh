...
Service_lightsocks(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/ihomelp07/doubi/master/other/lightsocks_centos" -O /etc/init.d/lightsocks; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/ihomelp07/doubi/master/other/lightsocks_debian" -O /etc/init.d/lightsocks; then
			echo -e "${Error} Lightsocks服务 管理脚本下载失败 !" && rm -rf "${file}" && exit 1
		fi
		chmod +x "/etc/init.d/lightsocks"
...
Update_Shell(){
	...
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/ihomelp07/doubi/master/lightsocks.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	if [[ ${sh_new_type} == "softs" ]]; then
		...
		wget -N --no-check-certificate "https://raw.githubusercontent.com/ihomelp07/doubi/master/lightsocks.sh" && chmod +x lightsocks.sh
	fi
