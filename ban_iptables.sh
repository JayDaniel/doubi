...
Update_Shell(){
	...
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/ihomelp07/doubi/master/ban_iptables.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	...
	if [[ ${sh_new_type} == "softs" ]]; then
		...
		wget -N --no-check-certificate "https://raw.githubusercontent.com/ihomelp07/doubi/master/ban_iptables.sh" && chmod +x ban_iptables.sh
	fi
