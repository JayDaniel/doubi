...
Service_caddy(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/JayDaniel/doubi/master/other/caddy_centos -O /etc/init.d/caddy; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/JayDaniel/doubi/master/other/caddy_debian -O /etc/init.d/caddy; then
			echo -e "${Error_font_prefix}[错误]${Font_suffix} Caddy服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/caddy
