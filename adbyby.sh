...
Download_adbyby(){
	...
	cd bin
	chmod 777 adbyby
	wget -N --no-check-certificate "https://raw.githubusercontent.com/ihomelp07/doubi/master/other/adhook.ini"
}
Service_adbyby(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/ihomelp07/doubi/master/other/adbyby_centos -O /etc/init.d/adbyby; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/ihomelp07/doubi/master/other/adbyby_debian -O /etc/init.d/adbyby; then
			echo -e "${Error} ADbyby服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/adbyby
