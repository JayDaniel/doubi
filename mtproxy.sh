...
Service_mtproxy(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/ihomelp07/doubi/master/other/mtproxy_centos" -O /etc/init.d/mtproxy; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/ihomelp07/doubi/master/other/mtproxy_debian" -O /etc/init.d/mtproxy; then
			echo -e "${Error} MTProxy服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x "/etc/init.d/mtproxy"
...
Update_Shell(){
	...
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/ihomelp07/doubi/master/mtproxy.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	if [[ ${sh_new_type} == "softs" ]]; then
		...
		wget -N --no-check-certificate "https://raw.githubusercontent.com/ihomelp07/doubi/master/mtproxy.sh" && chmod +x mtproxy.sh
	fi
